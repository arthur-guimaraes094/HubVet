        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
          <div className="flex flex-col gap-2">
            ...
            {isEdit ? (
              ...
              >
                ...
                {patients.map(p => (
                  <option className="bg-card text-foreground" key={p.id} value={p.id}>{p.name} ({translateSpecies(p.species)})</option>
                ))}
              </select>
            )}
          </div>
          ...
          <div className="flex gap-4 mt-2">
            <Button 
              ...
              type="button" 
              onClick={handleClose}
              className="py-5 flex-1 font-black uppercase tracking-widest text-[10px]"
            >
              Cancelar
            </Button>
