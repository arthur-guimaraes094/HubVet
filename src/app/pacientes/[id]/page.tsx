  ...
  return (
    <ViewTransition enter="fade-in" default="none">
      <main className="flex-1 flex flex-col p-8 gap-8 w-full max-w-4xl mx-auto">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          <div className="flex gap-4">
            <Link href={`/prontuario?id=${patient.id}&type=Paciente`}>
              <Button variant="primary" className="!px-8 !py-3 shadow-sm border border-border hover:shadow-sm border border-border">
                Novo Atendimento
              </Button>
            </Link>
            <Link href="/pacientes">
        ...
        {/* Gráfico de Peso */}
        {weightData.length > 1 && (
          <Card className="p-6 flex flex-col gap-4 bg-background shadow-sm border border-border border border-foreground/5 overflow-hidden">
              ...
              {consultations.map((consult) => {
                ...
                return (
                  <div key={consult.id} className="relative pl-12">
                    {/* Timeline Dot */}
                    <div className={`absolute left-0 top-2 w-10 h-10 bg-background rounded-full shadow-sm border border-border flex items-center justify-center z-10 border border-foreground/5 ${isScheduled ? 'animate-pulse border-primary/30' : ''}`}>
                      ...
                      {isScheduled ? (
                        ...
                        <>
                          <div className="space-y-2">
                            ...
                            <p className="text-sm text-foreground/80 leading-relaxed bg-background/50 p-4 rounded-xl shadow-inner border border-border bg-gray-50/50 border border-foreground/5 italic">
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {consult.images.map((url: string, idx: number) => (
                                  <div key={idx} className="relative w-20 h-20 rounded-xl overflow-hidden shadow-sm border border-border bg-gray-50/50 border border-foreground/5 group cursor-pointer hover:scale-105 transition-transform">
                                    <Image 
                                      src={url} 
                                      alt={`Anexo ${idx + 1}`} 
                                      fill 
                            ...
                            <div className="space-y-3">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {consult.consultation_items.map((item: ConsultationItem) => (
                                  <div key={item.id} className="px-3 py-1 bg-background shadow-sm border border-border rounded-lg text-xs font-medium text-foreground flex items-center gap-2 border border-foreground/5">
